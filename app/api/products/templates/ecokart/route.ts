...
export async function GET() {
  try {
    ...
    const response = await fetch('https://www.ecokartuk.com/api/products/enhanced?take=100');
    if (!response.ok) {
    ...
    const liveProducts = Array.isArray(apiResult) ? apiResult : apiResult.products;
    ...
    const excelData = liveProducts.map((p: any) => ({
        'SKU': p.sku,
        'Name': p.name,
        'Description': p.description || p.name, // Use name as fallback for description
